...
Tile* TileMap::getTile(const int x, const int y, const int z) const
{
	if (x >= 0 && x < this->maxSizeWorldGrid.x &&
		y >= 0 && y < this->maxSizeWorldGrid.y &&
		z >= 0 && z < this->layers)
...
Tile* TileMap::getTile(const int x, const int y) const
{
	if (x >= 0 && x < this->maxSizeWorldGrid.x &&
		y >= 0 && y < this->maxSizeWorldGrid.y)
	{
		if (!this->map[x][y].back().empty())
		{
			return this->map[x][y].back().back();
		}
	}
...
void TileMap::updateTiles(Entity* entity, const float& dt)
{
	...
	this->toX = entity->getGridPosition(this->gridSizeI).x + 11;
	if (this->toX < 0)
	{
		this->toX = 0;
	}
	...
	this->toY = entity->getGridPosition(this->gridSizeI).y + 11;
	if (this->toY < 0)
	{
		this->toY = 0;
	}
...
void TileMap::updateCapturing(Entity* entity, const float& dt)
{
	int32_t counter = 0;
		...
		for (int y = 0; y < this->maxSizeWorldGrid.y; y++)
		{
			for (int z = 0; z < this->layers; z++)
			{
				if (!this->map[x][y][z].empty())
				{
					for (size_t k = 0; k < this->map[x][y][z].size(); k++)
					{
						this->map[x][y][z][k]->update(dt);

						if (this->map[x][y][z][k]->getDistance(entity) < this->gridSizeF / 3.f)
						{
							++counter;
						}
						...
						if (counter > 0)
						{
							this->map[x][y][z][k]->setCaptured();
						}
						else
						{
							this->map[x][y][z][k]->setCaptured(false);
						}
					}
				}
			}
